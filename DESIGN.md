...
| INV-04 basis-scoped revocation (multi-root composition) | ✅ landed — SPEC §6b.4 normative (refines INV-04) | effective authority = union over currently-valid authorization bases; `revoke(basis-X)` removes exactly basis-X's derivable authority (no less — full transitive closure; no more — other bases' contribution survives); MUST NOT reduce a subject to a global per-subject revoked/authorized bit (forbids over-/under-revocation); a surviving basis MUST NOT preserve authority unique to a revoked lineage |
